...
export default function Scenarios() {
    ...
    const visible         = scenarios.filter((s) => filter === "all" || s.difficulty === filter);
    const myScenarios     = userRole === "teacher" ? visible.filter((s) => s.created_by === userId) : [];
    const classFiltered   = visible.filter((s) => selectedClass === "all" || (s.class_names || []).includes(selectedClass));
                ...
                {!loading && !error && (
                    <>
                        ...
                        <section className="sc-section sc-section--delay2">
                            <SectionLabel label={userRole === "teacher" ? "All Scenarios" : "Your Scenarios"} count={classFiltered.length} />
                            {classFiltered.length > 0 ? (
                                ...
                                <div className="sc-student-empty">
                                    ...
                                    <p className="sc-student-empty__desc">Join a class using the enrolment code your teacher gave you to unlock your scenarios.</p>
                                    <button className="sc-btn sc-btn--primary" style={{ background: "#4cc9f0", color: "#000", boxShadow: "0 4px 20px #4cc9f044" }} onClick={() => setShowJoin(true)}>
                                        + Join a Class
                                    </button>
                                </div>
                            ) : (
